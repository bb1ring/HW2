...
I imported these three datasets and cleaned the variables' names. Then, I went through each dataset to see whether their variables were arranged in a reasonable way and if there were any differences among them. I noticed that only the 'bakers' dataset included the full names of bakers. I extract the first name so that I can merge these dataset by name. Then, I checked the completeness and correctness across data sets by using 'anto_join'. There was one baker's name was inconsistent in bakes dataset, so I correct the name. Finally, I merged datasets by using 'left_join', and relocated some variables.
